...
$query = mysqli_query($result,'select p.cliente_id, p.pedido_id as pedido_id, c.nombres as nombres, p.nombre_pedido, p.t_costo, p.fecha, p.estado, p.fecha_viaje as fecha_viaje from pedidos p inner join clientes c on p.cliente_id = c.id order by p.pedido_id;');
