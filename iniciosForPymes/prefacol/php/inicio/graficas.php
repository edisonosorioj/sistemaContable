...
$ingreso = mysqli_query($result,'SELECT SUM(valor) as total FROM ingresos WHERE YEAR(fecha) = YEAR(now()) AND MONTH(fecha) = MONTH(now());');
 	$cam1 = $ingreso->fetch_assoc();
 	$ingmes = $cam1['total'];

 $egreso = mysqli_query($result,"SELECT SUM(valor) as total FROM compras WHERE YEAR(fecha) = YEAR(now()) AND MONTH(fecha) = MONTH(now());");
