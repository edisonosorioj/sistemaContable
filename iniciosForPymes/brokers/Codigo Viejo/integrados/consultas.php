...
class Consulta{
    ...
    public static function Comodidades_Propiedad($mysqli,$codigo_propiedad){
        if ($consulta = mysqli_query($mysqli,"SELECT b.nombre as nombre, b.icon as icon FROM tbl_propiedad_comodidad_propiedad a INNER JOIN tbl_comodidad_general_propiedad b ON a.codigo_comodidad_propiedad = b.codigo WHERE a.codigo_propiedad = '".$codigo_propiedad."' ")) {
            if ($resultado = mysqli_fetch_array($consulta)) {
                $inicio = 0;
                do{
                    $comodidades_propiedad[$inicio] = $resultado['nombre'];
                    $inicio++;
    ...
    public static function Comodidades_Copropiedad($mysqli,$codigo_copropiedad){
        if ($consulta = mysqli_query($mysqli,"SELECT codigo_comodidad_copropiedad FROM tbl_copropiedad_comodidades_copropiedad WHERE codigo_comodidad_copropiedad = '".$codigo_copropiedad."' ")) {
            if ($resultado = mysqli_fetch_array($consulta)) {
                $inicio = 0;
                do{
                    $comodidades_propiedad[$inicio] = $resultado['codigo_comodidad_copropiedad'];
                    $inicio++;
                }while ($resultado = mysqli_fetch_array($consulta));
                return $comodidades_copropiedad;
            }
        }
    }
    ...
    public static function Informacion_Propiedad($mysqli,$codigo_propiedad){
        if ($consulta = mysqli_query($mysqli,"SELECT tbl_propiedad.titulo, tbl_propiedad.descripcion, tbl_sector_barrio.nombre as 'sector_barrio', tbl_propiedad.direccion, tbl_propiedad.area_total, tbl_propiedad.area_construida, tbl_propiedad.fecha_registro_propiedad, tbl_tipo_propiedad.nombre as 'tipo_propiedad', tbl_propiedad.estrato, tbl_propiedad.numero_niveles, tbl_propiedad.numero_piso, tbl_propiedad.numero_alcoba, tbl_propiedad.numero_bano, tbl_propiedad.video, tbl_propiedad.parqueadero, tbl_propiedad.cuarto_util_parqueadero, tbl_propiedad.valor_arriendo, tbl_propiedad.valor_venta, tbl_propiedad.tipo_cocina, tbl_propiedad.tipo_piso, tbl_propiedad.constructora, tbl_propiedad.copropiedad, tbl_propiedad.nombre_carpeta, tbl_propiedad.codigo as 'codigo_propiedad', tbl_propiedad.sector_barrio as 'sector_propiedad', tbl_ciudad.nombre as 'nombre_ciudad', tbl_provincia.nombre as 'nombre_provincia', tbl_pais.nombre as 'nombre_pais' FROM tbl_propiedad INNER JOIN tbl_sector_barrio ON tbl_propiedad.sector_barrio = tbl_sector_barrio.codigo INNER JOIN tbl_tipo_propiedad ON tbl_propiedad.tipo_propiedad = tbl_tipo_propiedad.codigo INNER JOIN tbl_ciudad ON tbl_sector_barrio.ciudad = tbl_ciudad.codigo INNER JOIN tbl_provincia ON tbl_ciudad.provincia = tbl_provincia.codigo INNER JOIN tbl_pais ON tbl_provincia.pais = tbl_pais.codigo
            ...
            if ($resultado = mysqli_fetch_array($consulta)) {
                ...
                $informacion_propiedad[25] = $resultado['nombre_ciudad'];
                $informacion_propiedad[26] = $resultado['nombre_provincia'];
                $informacion_propiedad[27] = $resultado['nombre_pais'];
