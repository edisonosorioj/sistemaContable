<?php
	...
	*/	
	...
	if( !class_exists('kodeproperty_font_loader_plugin') ){	
		class kodeproperty_font_loader_plugin{
			...
			function __construct( $custom_font ){
				$this->kodeproperty_set_google_font_list();
				// $this->kodeproperty_set_custom_font_list($custom_font);
				
				// add filter to include font into the theme
				add_filter('kodeproperty_enqueue_scripts', array(&$this, 'kodeproperty_include_google_font'));
				add_filter('kodeproperty_style_custom_end', array(&$this, 'add_custom_style'), 1, 2);

				add_action('kodeproperty_print_all_font_list', array(&$this, 'kodeproperty_print_all_font_list'));
			}
			
			// get the custom font list to array
			function kodeproperty_set_custom_font_list($custom_fonts){
				$this->custom_font_list = array(
					'Dejavu Sans Condense' => array(
						'ttf' => KODEPROPERTY_PATH_URL . '/stylesheet/DejaVuSansCondensed.ttf',
						'eot' => KODEPROPERTY_PATH_URL . '/stylesheet/DejaVuSansCondensed.eot'
					)
				);
			...
			function kodeproperty_set_google_font_list(){
				if( is_admin() ){
					$google_font_array = '{
  ...
  },
  {
   ...
   "files": {
    "regular": "http://themes.googleusercontent.com/static/fonts/zeyada/v3/hmonmGYYFwqTZQfG2nRswQ.ttf"
					...
					$google_font_file = apply_filters('kodeproperty_google_font_file', $google_font_array);
					$google_fonts = json_decode($google_font_file, true);
					...
					$this->google_font_list = get_option('kodeproperty_google_font_list');
					
				}
			}
			
			// get all font list to admin panel area
			function kodeproperty_print_all_font_list( $selected_font ){

				// safe font section
				echo '<option disabled >------ ' . esc_html__('Default Fonts' ,'kode-property') . ' ------</option>';
				...
				// google font section
				echo '<option disabled >------- ' . esc_html__('Google Font' ,'kode-property') . ' -------</option>';
				foreach( $this->google_font_list as $font_family => $font ){
					echo '<option data-type="google-font" ';
					echo 'data-url="' . $this->kodeproperty_get_google_font_url($font_family) . '" ';
			...
			function kodeproperty_get_google_font_url( $font_family ){
				if( !empty($font_family) && !empty($this->google_font_list[$font_family]) ){
					...
					$temp_font_name .= apply_filters('kodeproperty_google_font_weight', implode(',', $google_font['variants'])) . '&subset='; 
					$temp_font_name .= apply_filters('kodeproperty_google_font_subset', implode(',', $google_font['subsets'])); 
			...
			function add_custom_style($kodeproperty_plugin_option){
				$used_font = array();
				foreach( $this->font_location as $location ){
					$current_font = $kodeproperty_plugin_option[$location];
			...
			function kodeproperty_include_google_font( $array ){
				...
				foreach( $used_font as $font_name ){
					...
					$array['style'][$font_id . '-google-font'] = $this->kodeproperty_get_google_font_url($font_name);
				}	
